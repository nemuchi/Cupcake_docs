<?php
	...
	function viewCus($cusno){
		...
		$admin = new Admin();
			$cus = $admin->getCustomer($adstall_no);
			...
			while($row = mysql_fetch_array($ret_cus)){
    			...
    			echo "<div id=cusimg>";
  				echo "<img src='img/Customer/".$fname.".jpg'/>";
 			    echo "</div>
			...
			//"SELECT i.category,i.item_name,t.quantity,i.price,t.date_time,t.type
			...
			//				c.cust_no='$cusno' and s.stall_no='$adstall_no'";
	...
	function dispCustomer(){
		...
		if(isset($_POST['search'])){
			...
			$c_query = $admin->searchCustomer($adstall_no,$csearch);
		}else{
			$c_query = $admin->getDistinctCustomer($adstall_no);
		}
			$ret = mysql_query($c_query);
			while($row = mysql_fetch_array($ret)){
				$fname=$row['fname'];
				$mname=$row['mname'];
				$lname=$row['lname'];
				echo "<tr id=result bgcolor=#CCC onClick=location.href='admin_viewCustomer.php?cust_no=".$row['cust_no']."'>";
				echo "<td>".$fname." ".$mname." ".$lname."</td>";
				$customer = new Customer();
			   	$cc = $customer->countTransaction($row['cust_no']);
...
<div id=customer>
	<div id=cust_info><h1><?php echo $_SESSION['adstall_name'];?>' Customers</h1></div>
    <div id=cust_links>
    	<div id=search_form>
